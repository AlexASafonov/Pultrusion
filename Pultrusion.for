...
C
      DIMENSION TIME(2)
C
      DOUBLE PRECISION :: k_min_al_1, k_min_al_2, k_max_T_1, k_max_T_2,
     1 k_max_S_1, k_max_S_2, k_Strength, k_smax(22), k_Str(22), 
     2 k_smaxT(22)  
...
C           write(105,'(F12.6)') k_T(i)

C           write(105,'(F12.2)') k_S(i)

C         write(105,'(F12.6)') TIME(2)
         
C----- Al            

      IF (k_min_al_1.GT.k_min_al_2) THEN
    
            k_min_al_2=k_min_al_1
            
      END IF
      
C        write(105,'(F12.6)') k_min_al_1   
...
C        write(105,'(F12.6)') k_max_T_2
        
C         write(105,'(F12.6)') k_max_T_1                  
...
C      IF (k_max_S_1.GT.k_max_S_2) THEN
    
C           k_max_S_2=k_max_S_1
            
C      END IF      
      
C      write(105,'(F12.2)') k_max_S_1
...
C Function for calculation of matrix Young modulus
       ...
       MATRIX_YOUNG=E101+(TEMP-T101)/(T80-T101)*(E80-E101)
       GOTO 200
       ENDIF
  
       IF ((TEMP.GT.T80).AND.(TEMP.LE.T60)) THEN
       MATRIX_YOUNG=E80+(TEMP-T80)/(T60-T80)*(E60-E80)
       GOTO 200
       ENDIF
 
       IF ((TEMP.GT.T60).AND.(TEMP.LE.T40)) THEN
       MATRIX_YOUNG=E60+(TEMP-T60)/(T40-T60)*(E40-E60)
       GOTO 200
       ENDIF 
       
       IF ((TEMP.GT.T40).AND.(TEMP.LE.T20)) THEN
       MATRIX_YOUNG=E40+(TEMP-T40)/(T20-T40)*(E20-E40)
       GOTO 200
       ENDIF 
